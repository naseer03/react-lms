...
const StudentLayout = () => {
  ...
  const navLinkClass = ({ isActive }) =>
    `flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 ${
      isActive ? 'bg-primary-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-800'
    }`;
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 z-40 flex">
          ...
          <aside className="relative w-64 bg-white flex flex-col shadow-xl">
            <button onClick={() => setSidebarOpen(false)} className="absolute top-4 right-4 p-1.5 rounded-lg hover:bg-slate-100">
              <X size={18} />
            </button>
            <Sidebar />
          </aside>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b border-slate-100 px-6 py-4 flex items-center">
          <button className="lg:hidden p-2 rounded-lg hover:bg-slate-100 mr-3" onClick={() => setSidebarOpen(true)}>
            <Menu size={20} />
          </button>
          <h1 className="text-base font-semibold text-slate-700">Welcome back, {user?.name?.split(' ')[0]}!</h1>
        </header>
